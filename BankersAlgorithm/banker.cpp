...
void printTableWork(int Seq[],int work[][maxR]){
	for (int i = 0; i < p ; i++)
		cout << "P" << Seq[i] << " ";
	
	cout<<endl<<setw(25)<<"TABLE WORK"<<endl;
	cout<<setw(6);
	for (int i = 0; i < p + 1; i++){
		if (i != 0)
			cout << "P" << Seq[i-1] <<setw(5) << "|"<<setw(5);
		else	
			cout <<"avail"<<setw(5)<<"|"<<setw(5);	
		for (int j = 0; j< r; j++)
			cout << work[i][j]<<setw(5)<< "|"<<setw(5);
		cout<<endl;
	}
}

void printTableNeed(int Seq[],int need[][maxR]){

	cout<<endl<<setw(25)<<"TABLE NEED"<<endl;
	cout<<setw(5);
	for (int i = 0; i < p; i++){
		cout << "P" << Seq[i] <<setw(5)<<"|"<<setw(5);
		for (int j = 0; j< r; j++)
			cout << need[i][j]<<setw(5)<<"|"<<setw(5);
		cout<<endl;
	}
}
void banker1(){
	...
	printTableWork(safeSeq,work);
	printTableNeed(safeSeq,need);
}
